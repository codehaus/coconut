 ...
 * Java types are mappable (currently required by the platform MXBeans): 1.
 * Primitive types 2. Wrapper classes such java.lang.Integer, etc 3. Classes
 * with only getter methods and with a static "from" method that takes a
 * CompositeData argument. 4. E[] where E is a type of 1-4 (can be
 * multi-dimensional array) 5. List<E> where E is a type of 1-3 6. Map<K, V>
 * where K and V are a type of 1-4 OpenDataException will be thrown if a Java
...
public abstract class MappedMXBeanType {
    ...
    static class BasicMXBeanType extends MappedMXBeanType {
        ...
        Object toJavaTypeData(Object data) throws OpenDataException,
    ...
    static class ArrayMXBeanType extends MappedMXBeanType {
        ...
        Object toOpenTypeData(Object data) throws OpenDataException {
            ...
            final Object[] openArray = (Object[]) Array.newInstance(
                    componentType.getMappedTypeClass(), array.length);
            int i = 0;
            for (Object o : array) {
                if (o == null) {
                    openArray[i] = null;
        ...
        Object toJavaTypeData(Object data) throws OpenDataException,
            ...
            final Object[] array = (Object[]) Array.newInstance(
                    (Class) componentType.getJavaType(), openArray.length);
            int i = 0;
    ...
    static class ListMXBeanType extends MappedMXBeanType {
        ...
        Object toOpenTypeData(Object data) throws OpenDataException {
            ...
            final Object[] openArray = (Object[]) Array.newInstance(paramType
                    .getMappedTypeClass(), list.size());
            int i = 0;
            for (Object o : list) {
    ...
    static class MapMXBeanType extends MappedMXBeanType {
        ...
        MapMXBeanType(ParameterizedType pt) throws OpenDataException {
            ...
            final OpenType[] mapItemTypes = new OpenType[] {
                    keyType.getOpenType(), valueType.getOpenType(), };
            final CompositeType rowType = new CompositeType(typeName, typeName,
                    mapItemNames, mapItemNames, mapItemTypes);

            openType = new TabularType(typeName, typeName, rowType,
                    mapIndexNames);
        ...
        Object toOpenTypeData(Object data) throws OpenDataException {
            ...
            for (Map.Entry entry : map.entrySet()) {
                ...
                final CompositeData row = new CompositeDataSupport(rowType,
                        mapItemNames, new Object[] { key, value });
                table.put(row);
            }
            return table;
        }
    ...
    static class CompositeDataMXBeanType extends MappedMXBeanType {
        ...
        CompositeDataMXBeanType(Class c) throws OpenDataException {
            ...
            try {
                fromMethod = (Method) AccessController
                        .doPrivileged(new PrivilegedExceptionAction() {
                            public Object run() throws NoSuchMethodException {
                                return javaClass.getMethod("from",
            ...
            if (COMPOSITE_DATA_CLASS.isAssignableFrom(c)) {
                ...
                final Method[] methods = (Method[]) AccessController
                 ...
                 * isX()", where T is not void and X is not the empty string.
                 * Exclude "Class getClass()" inherited from Object.
                 */
                for (int i = 0; i < methods.length; i++) {
                    ...
                    } else if (name.startsWith("is") && type instanceof Class
                ...
                final OpenType[] typeArray=types.toArray(new OpenType[0]);
                System.out.println(c.getName());
                System.out.println(Arrays.toString(nameArray));
                System.out.println(Arrays.toString(typeArray));
                openType = new CompositeType(c.getName(), c.getName(),
        ...
        Object toOpenTypeData(Object data) throws OpenDataException {
            ...
            // return MemoryUsageCompositeData.toCompositeData((MemoryUsage)
            // data);
            // }
            //    
            // if (data instanceof java.lang.management.ThreadInfo) {
            // return ThreadInfoCompositeData.toCompositeData((ThreadInfo)
            // data);
            // }
            //    
            // if (data instanceof java.lang.management.LockInfo) {
            // if (data instanceof java.lang.management.MonitorInfo) {
            // return MonitorInfoCompositeData.toCompositeData((MonitorInfo)
            // data);
            // }
            ...
            if (isCompositeData) {
                ...
                String[] itemNames = (String[]) ct.keySet().toArray(
                        new String[0]);
        ...
        Object toJavaTypeData(Object data) throws OpenDataException,
            ...
            try {
                ...
                final OpenDataException ode = new OpenDataException(
                        "Failed to invoke " + fromMethod.getName()
                                + " to convert CompositeData " + " to "
                                + javaClass.getName());
                ode.initCause(e);
                throw ode;
            }
            throw new IllegalStateException("jd");
        }
    }
    ...
    private static final OpenType[] simpleTypes = { BIGDECIMAL, BIGINTEGER,
            BOOLEAN, BYTE, CHARACTER, DATE, DOUBLE, FLOAT, INTEGER, LONG,
    ...
    static {
        try {
            for (int i = 0; i < simpleTypes.length; i++) {
                ...
                Class c = null;
                try {
                    c = Class.forName(t.getClassName(), false, String.class
                            .getClassLoader());
