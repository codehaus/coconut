...
public abstract class AbstractStageManager implements StageManager {
     ...
     */
    private volatile ThreadFactory threadFactory = Executors
     ...
     * @return
     */
    protected ThreadWorker addThreadWorker(ThreadFactory threadFactory) {
        final ReentrantLock mainLock = this.mainLock;
        mainLock.lock();
        try {
            Worker w = new Worker();
    ...
    public void shutdown() {
        ...
        try {
            if (allWorkers.size() > 0) {
                ...
                RunState state = runState;
                if (state == RunState.RUNNING) // don't override shutdownNow
     ...
     *             denies access.
     */
    public Map<Stage, List> shutdownNow() {
    ...
    /**
     * Invokes <tt>shutdown</tt> when this stagemanager is no longer
     * referenced.
     */
    ...
    /**
     * @see org.coconut.event.sedaold.impl.AbstractStageManager#remainingEvents()
     */
    protected Map<Stage, List> remainingEvents() {
        Map<Stage, List> map = new HashMap<Stage, List>();
        for (Stage s : getStages()) {
            map.put(s, Collections.EMPTY_LIST);
        }
        return map;
    }
     ...
     */
    protected static enum RunState {
     ...
     * @param event
     *            the event that the stage was processing when the failure
     *            occured
     * @param exception
     *            the exception that was thrown
     * @return
     */
    protected Object stageFailed(Stage s, Object event, Throwable t) {
        return null;
    }
}
