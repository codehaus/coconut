 ...
 * entail locking, and there is <em>not</em> any support for locking the
 * entire table in a way that prevents all access. This class is fully
 * interoperable with <tt>Hashtable</tt> in programs that rely on its thread
 ...
 */
public class ConcurrentPocketMap<K, V> extends AbstractMap<K, V> implements
        ConcurrentMap<K, V> {
    //TODO Figure out to keep 
    //When we load a value size is not updated until the value has been loaded.
    //What do we do with the HashEntries??
    //rehash discards some of them
    //So we probably need some kind of value holder/
    //get -> loadValue ->
    //insert future stuff into segment.
     ...
     * user-visible Map.Entry. Because the value field is volatile, not final,
     * it is legal wrt the Java Memory Model for an unsynchronized reader to see
     * null instead of initial value when read via a data race. Although a
     * reordering leading to this is not likely to ever actually occur, the
     * Segment.readValueUnderLock method is used as a backup in case a null
    ...
    static final class Segment<K, V> extends ReentrantLock implements Serializable {
         ...
         * the default load factor threshold.) Read operations can thus proceed
         * without locking, but rely on selected uses of volatiles to ensure
         * that completed write operations performed by other threads are
         * noticed. For most purposes, the "count" field, tracking the number of
         * elements, serves as that volatile variable ensuring visibility. This
         * is convenient because this field needs to be read in many read
         * operations anyway: - All (unsynchronized) read operations must first
         * read the "count" field, and should not look at table entries if it is
         * 0. - All (synchronized) write operations should write to the "count"
         ...
         * reads. As a guide, all critical volatile reads and writes to the
     ...
     */
    public ConcurrentPocketMap(int initialCapacity, float loadFactor, int concurrencyLevel) {
     ...
     * @since 1.6
     */
    public ConcurrentPocketMap(int initialCapacity, float loadFactor) {
     ...
     */
    public ConcurrentPocketMap(int initialCapacity) {
     ...
     *            the map
     */
    public ConcurrentPocketMap(Map<? extends K, ? extends V> m) {
    ...
    abstract class HashIterator {
        ...
        final void advance() {
            ...
            while (nextTableIndex >= 0) {
                if ((nextEntry = currentTable[nextTableIndex--]) != null) {
                    if (nextEntry.isVisible() || advanceEntry()) {
                        return;
                    }
                }
            }
                ...
                if (seg.count != 0) {
                    ...
                    for (int j = currentTable.length - 1; j >= 0; --j) {
                        if ((nextEntry = currentTable[j]) != null
                                && (nextEntry.isVisible() || advanceEntry())) {
                            nextTableIndex = j - 1;
                            return;
                        }
                    }
        ...
        final boolean advanceEntry() {
            if (nextEntry != null) {
                while ((nextEntry = nextEntry.next) != null && !nextEntry.isVisible())
                    ;
            }
        ...
        public void remove() {
            ...
            ConcurrentPocketMap.this.remove(lastReturned.key);
            lastReturned = null;
        }
    }
     ...
     */
    final class WriteThroughEntry extends org.coconut.internal.util.SimpleEntry<K, V> {
        ...
        public V setValue(V value) {
            ...
            ConcurrentPocketMap.this.put(getKey(), value);
            return v;
        }
    }
    ...
    final class KeySet extends AbstractSet<K> {
        ...
        public int size() {
            return ConcurrentPocketMap.this.size();
        }

        public boolean contains(Object o) {
            return ConcurrentPocketMap.this.containsKey(o);
        }

        public boolean remove(Object o) {
            return ConcurrentPocketMap.this.remove(o) != null;
        }

        public void clear() {
            ConcurrentPocketMap.this.clear();
        }
    }
    ...
    final class Values extends AbstractCollection<V> {
        ...
        public boolean contains(Object o) {
            return ConcurrentPocketMap.this.containsValue(o);
        }
    ...
    final class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean contains(Object o) {
            ...
            V v = ConcurrentPocketMap.this.get(e.getKey());
        ...
        public boolean remove(Object o) {
            if (!(o instanceof Map.Entry))
                return false;
            Map.Entry<?, ?> e = (Map.Entry<?, ?>) o;
            return ConcurrentPocketMap.this.remove(e.getKey(), e.getValue());
        }
