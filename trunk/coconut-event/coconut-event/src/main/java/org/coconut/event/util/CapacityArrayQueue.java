 ...
 * queue constructed with fairness set to <tt>true</tt> grants threads access
 * in FIFO order. Fairness generally decreases throughput but reduces
 ...
 */
public class CapacityArrayQueue<E> extends AbstractQueue<E> implements CapacityQueue<E>,
     ...
     */
    public CapacityArrayQueue(int capacity) {
        this(capacity, false);
    }
     ...
     *             if <tt>capacity</tt> is less than 1
     */
    public CapacityArrayQueue(int capacity, boolean fair) {
        if (capacity <= 0)
            throw new IllegalArgumentException();
        this.capacity = capacity; // MOD
     ...
     * @throws IllegalArgumentException
     *             if <tt>capacity</tt> is less than <tt>c.size()</tt>, or
     *             less than 1.
     * @throws NullPointerException
     *             if the specified collection or any of its elements are null
     */
    public CapacityArrayQueue(int capacity, boolean fair, Collection<? extends E> c) {
        this(capacity, fair);
        if (capacity < c.size())
            throw new IllegalArgumentException();
     ...
     * Inserts the specified element at the tail of this queue if it is possible
     * to do so immediately without exceeding the queue's capacity, returning
     * <tt>true</tt> upon success and <tt>false</tt> if this queue is full.
     * This method is generally preferable to method {@link #add}, which can
    ...
    public void setCapacity(int newCapacity, boolean downSize) {
        if (capacity < 0) {
            throw new IllegalArgumentException();
        }    
        final ReentrantLock lock = this.lock;
        ...
        try {
            ...
            // drainTo calls notFull.signalAll();
            // we might want to refactor this, so its only called
            // if there is extra room available
            drainTo(elements);
            int newSize = Math
                    .max(newCapacity, downSize ? elements.size() : items.length);
            //see if we can reuse array
            Object[] newItems = items.length == newSize ? items : new Object[newSize];
            for (E e : elements) {
                newItems[putIndex] = e;
                putIndex = inc(putIndex);
            }
     ...
     * element will succeed by inspecting <tt>remainingCapacity</tt> because
     * it may be the case that another thread is about to insert or remove an
     * element.
     */
     ...
     * <tt>o.equals(e)</tt>, if this queue contains one or more such
     * elements. Returns <tt>true</tt> if this queue contained the specified
    ...
    public int drainTo(Collection<? super E> c, long timeout, TimeUnit unit) throws InterruptedException {
        if (c == null)
            throw new NullPointerException();
        if (c == this)
            throw new IllegalArgumentException();
    ...
    private class Itr implements Iterator<E> {
        ...
        public E next() {
            final ReentrantLock lock = CapacityArrayQueue.this.lock;
            lock.lock();
            try {
                if (nextIndex < 0)
