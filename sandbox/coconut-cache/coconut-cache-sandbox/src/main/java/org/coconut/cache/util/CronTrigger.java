...
public class CronTrigger {
    ...
    public static Runnable ClearCache(String cronExpression) {
        return null;
    }

    /**
     * Provides a parser and evaluator for unix-like cron expressions. Cron
     * expressions provide the ability to specify complex time combinations such
     * as &quot;At 8:00am every Monday through Friday&quot; or &quot;At 1:30am
     ...
     * is not complete (you'll need to use the '?' character in on of these
     * fields). </li>
     * </ul>
     * </p>
     * 
    ...
    static class CronExpression implements Serializable, Cloneable {
        ...
        public Date getNextInvalidTimeAfter(Date date) {
            ...
            // TODO: (QUARTZ-481) IMPROVE THIS! The following is a BAD solution
            // to this problem. Performance will be very bad here, depending on
            // the cron expression. It is, however A solution.
        ...
        /**
         * Sets the time zone for which this <code>CronExpression</code> will
         * be resolved.
         */
                ...
                if (type == MONTH) {
                    ...
                    if (s.length() > i + 3) {
                        ...
                        if (c == '-') {
                            ...
                            if (sval > eval) {
                                throw new ParseException("Invalid Day-of-Week sequence: "
                                        + sval + " > " + eval, i);
                            }
                        } else if (c == '#') {
                            try {
                    ...
                    throw new ParseException("Illegal characters for this position: '"
                            + sub + "'", i);
                }
                if (eval != -1) {
                    incr = 1;
        ...
        protected Date getTimeAfter(Date afterTime) {
            ...
            while (!gotOne) {
                ...
                if (dayOfMSpec && !dayOfWSpec) { // get day by day of month
                    // rule
                    ...
                    // week rule
                    if (lastdayOfWeek) { // are we looking for the last XXX
                        // day of
                        // the month?
                        ...
                        if (day + daysToAdd > lDay) { // did we already miss
                            // the
                            // last one?
                            cl.set(Calendar.SECOND, 0);
                            cl.set(Calendar.MINUTE, 0);
                        ...
                        if (day + daysToAdd > lDay) { // will we pass the end
                            // of
                            // the month?
                            cl.set(Calendar.SECOND, 0);
                            cl.set(Calendar.MINUTE, 0);
        ...
        protected int getLastDayOfMonth(int monthNum, int year) {
            //TODO change to
            Calendar cal = new GregorianCalendar(year, monthNum, 1);
