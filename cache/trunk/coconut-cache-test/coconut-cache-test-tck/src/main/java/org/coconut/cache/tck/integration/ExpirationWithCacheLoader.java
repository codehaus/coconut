...
public class ExpirationWithCacheLoader extends ExpirationTestBundle {
    ...
    public void loadingOfExpiredEntries() {
        expiration().put(M1.getKey(), "~A", 1, TimeUnit.NANOSECONDS);
        expiration().put(M2.getKey(), "~B", 1, TimeUnit.NANOSECONDS);
        expiration().put(M3.getKey(), "~C", 1, TimeUnit.NANOSECONDS);
        expiration().put(M4.getKey(), "~D", 1, TimeUnit.NANOSECONDS);
